...
function mp_deathmatch:init()
    ...
    ConsoleService:ExecuteCommand("display_player_stats 1")
	...
	self.srq:AddState( "check_event", { execute="OnCheckEventExecute", interval=20.0 } )
...
function mp_deathmatch:OnPreparationExit( state)
    ...
    self.srq:ChangeState("check_event")
    self.fsm:ChangeState("idle" )
end
...
function mp_deathmatch:OnCheckEventExecute()

    if (g_lastRand == 2) then
        ConsoleService:ExecuteCommand("time_of_day day")
    end
    
    if (g_lastRand == 6) then
        local players = PlayerService:GetPlayersMechs()
        for entity in Iter(players) do

            local scale = EntityService:GetScale( entity )
            if (scale.x ~= 1) then
                EntityService:SetScale(entity, 1, 1, 1)
            end
        end
    end

    local l_Randint = RandInt( 1, 10 )
    g_lastRand = l_Randint

    if (l_Randint == 2) then
        ConsoleService:ExecuteCommand("time_of_day night")
    end

    -- currently crashes the game 12.11.2023
    -- if (l_Randint == 4) then

    --     local l_artyAlive = FindService:FindEntityByBlueprint("units/ground/alien_tower_artillery")
    --     if (l_artyAlive == "") or (l_artyAlive == INVALID_ID) then
    --         EntityService:SpawnEntity( "units/ground/alien_tower_artillery", 0, 0, 0, "player" )
    --     end
    -- end
    ...
    if (l_Randint == 6) then
        ...
        local players = PlayerService:GetPlayersMechs()
        for entity in Iter(players) do
            local l_RandFloat = RandFloat( 0.5, 2.0 )

            local scale = EntityService:GetScale( entity )
            if (scale.x ~= 1) then
                EntityService:SetScale(entity, 1, 1, 1)
            else
                EntityService:SetScale(entity, l_RandFloat, l_RandFloat, l_RandFloat)
            end
        end
    end
    ...
    if (l_Randint == 8) then
        TornadoService:SpawnTornado( "weather/tornado_big", "target_player", "near_player", 2, 50, 15, 20 )
    end
