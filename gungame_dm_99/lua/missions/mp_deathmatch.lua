-- To-Do
-- CANT-FIX: Gui to player which shows information about mod (Not provided by EXOR)
...
function mp_deathmatch:init()
    ...
    local campaignDataSRQ = CampaignService:GetCampaignData()
    g_max_reward_level = campaignDataSRQ:GetIntOrDefault("max_reward_level", 4)
    g_min_player_for_start = campaignDataSRQ:GetIntOrDefault("min_player_for_start", 2)
    g_skipRewardUp = false
    g_skipRewardDown = false

    self.playerStatsSRQ = {}
    self.level_rewards = {
        [0] = {
            item_blueprint = "items/weapons/laser_sword_item",
            item_type = "melee",
            item_ammo = "",
            item_ammo_amount = ""
        },
        [1] = {
            item_blueprint = "items/weapons/bouncing_blades_item",
            item_type = "range",
            item_ammo = "ammo_dm_bouncing_blades",
            item_ammo_amount = 50
        },
        [2] = {
            item_blueprint = "items/weapons/grenade_launcher_item",
            item_type = "range",
            item_ammo = "ammo_dm_grenade_launcher",
            item_ammo_amount = 30
        },
        [3] = {
            item_blueprint = "items/weapons/minigun_item",
            item_type = "range",
            item_ammo = "ammo_dm_minigun",
            item_ammo_amount = 350
        },
        [4] = {
            item_blueprint = "items/weapons/railgun_item",
            item_type = "range",
            item_ammo = "ammo_dm_railgun",
            item_ammo_amount = 20
        }
    }
...
function mp_deathmatch:OnIdleStart( state)
    ...
    if ( self.campaignState == G_MP_MISSION_STATE_WARMUP and playersCount < g_min_player_for_start  )  then
        return
    end
...
function mp_deathmatch:OnIdleExecute( state, tick )
    if ( self.campaignState == G_MP_MISSION_STATE_WARMUP and self.timerSet == nil ) then
        ...
        if ( playersCount < g_min_player_for_start  )  then
            return
        end
...
function mp_deathmatch:OnPlayerCreatedEvent( event )
    ...
    if player_team == EntityService:GetTeam("player") or player_team == INVALID_TEAM then
        player_team = PlayerService:CreatePlayerTeam("player_" .. tostring(player_id), true)
        PlayerService:SetPlayerTeam(player_id, player_team)
    end
...
function mp_deathmatch:OnPlayerDiedEvent(evt)
    ...
    if (evt:GetKiller() ~= INVALID_ID) and (g_warmupover) then
        self:RewardLevelUp(evt:GetKiller(), evt:GetKillerName())
        self:RewardLevelDown(evt:GetPlayerId(), evt:GetPlayerName())
    end
...
function mp_deathmatch:addPlayerToStats(playerId, playerName)
    if not self.playerStatsSRQ[playerId] then
        self.playerStatsSRQ[playerId] = {
            playerId = playerId,
    ...
    --else
        --ConsoleService:Write("Player " .. playerName .. " with ID " .. playerId .. " already exists.")
    end
end

function mp_deathmatch:RewardLevelUp(playerId, playerName)
    self:addPlayerToStats(playerId, playerName)

    if (self.playerStatsSRQ[playerId].playerScore < g_max_reward_level) then
        self.playerStatsSRQ[playerId].playerScore = self.playerStatsSRQ[playerId].playerScore + 1
        g_skipRewardUp = false
    else
        g_skipRewardUp = true
    end

    self.playerStatsSRQ[playerId].playerRewardLevel = self.playerStatsSRQ[playerId].playerScore

    if ( self.playerStatsSRQ[playerId].playerRewardLevel ) and ( (self.playerStatsSRQ[playerId].playerRewardLevel) <= g_max_reward_level) then
        if not (g_skipRewardUp) then
            ...
            -- Add new reward item if the player isn't at g_max_reward_level with the playerScore
            local rewardItem = PlayerService:AddItemToInventory(playerId, self.level_rewards[self.playerStatsSRQ[playerId].playerRewardLevel].item_blueprint)
            PlayerService:EquipItemInSlot( playerId, rewardItem, "LEFT_HAND" )
        end

        -- Add the reward item ammo if it's a gun
        if (self.level_rewards[self.playerStatsSRQ[playerId].playerRewardLevel].item_type ~= "melee") then
            PlayerService:SetResourceAmount(self.level_rewards[self.playerStatsSRQ[playerId].playerRewardLevel].item_ammo, self.level_rewards[self.playerStatsSRQ[playerId].playerRewardLevel].item_ammo_amount, playerId )
        end

    -- else
    --     ConsoleService:Write("level_rewards not applied")
    end

end

function mp_deathmatch:RewardLevelDown(playerId, playerName)
    self:addPlayerToStats(playerId, playerName)

    if (self.playerStatsSRQ[playerId].playerScore > 0) then
        self.playerStatsSRQ[playerId].playerScore = self.playerStatsSRQ[playerId].playerScore - 1
        g_skipRewardDown = false
    else
        g_skipRewardDown = true
    end

    self.playerStatsSRQ[playerId].playerRewardLevel = self.playerStatsSRQ[playerId].playerScore
    if ( self.playerStatsSRQ[playerId].playerRewardLevel ) and ( (self.playerStatsSRQ[playerId].playerRewardLevel) <= g_max_reward_level) then
        if not (g_skipRewardDown) then
            -- Remove all old weapons
            local pawn = PlayerService:GetPlayerControlledEnt( playerId )
            local items = PlayerService:GetAllEquippedItemsInSlot("LEFT_HAND", playerId)
            for item in Iter(items) do
            ...
            -- Add new reward item if the player isn't at 0 with the playerScore
            local rewardItem = PlayerService:AddItemToInventory(playerId, self.level_rewards[self.playerStatsSRQ[playerId].playerRewardLevel].item_blueprint)
            PlayerService:EquipItemInSlot( playerId, rewardItem, "LEFT_HAND" )
        end
